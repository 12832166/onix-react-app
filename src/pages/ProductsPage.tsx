...
const ProductsPage = () => {
  ...
  }, [searchParams, setQuery, location.state, t]);
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const translatedDescription = t(`products.items.${product.id}.description`);
          ...
          {categories.map((category) => (
            <div key={category.id}>
              <button
                ...
                className={`w-full text-left font-semibold p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-black dark:text-white ${
                ...
                <div className="ml-4 mt-2 space-y-2">
                  {category.subcategories.map((sub) => (
                    <button
                      ...
                      className={`w-full text-left p-2 rounded text-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 ${
                        selectedSubcategory === sub.name ? 'bg-gray-100 dark:bg-gray-700' : ''
                      }`}
                    >
      ...
      <div className="flex-1">
        ...
        <div className="mb-6 flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-black dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
            >
        ...
        {sortedProducts.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400">{t('products.noResults')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {sortedProducts.map((product) => {
              ...
              return (
                <div key={product.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-2xl overflow-hidden group hover:shadow-2xl transition-shadow duration-300">
                  ...
                  <div className="p-4 flex flex-col h-[160px]">
                    <Link to={`/products/${product.id}`} className="flex-grow">
                      <h3 className="font-semibold text-lg mb-1 text-black dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors line-clamp-2">
                        {productName}
                      </h3>
                    </Link>
