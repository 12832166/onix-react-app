...
const HomePage = () => {
  ...
  const ProductCard = ({ product }: { product: typeof products[0] }) => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-2xl overflow-hidden group hover:shadow-2xl transition-shadow duration-300">
    ...
    <div className="space-y-12">
      ...
      <section className="mb-12">
        <div className="container mx-auto px-4 py-20 text-center">
          ...
          <p className="text-lg md:text-2xl mb-8 text-black dark:text-white">
            {t('home.discover')}
          </p>
          <button
            onClick={() => navigate('/products')}
            className="min-w-[220px] bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-black dark:text-white font-semibold py-3 px-8 rounded-full shadow-lg hover:scale-105 transition-colors duration-200"
          >
            {t('home.shopNow')}
          </button>
          <p className="mt-2 text-sm text-black/80 dark:text-white/80">
            {t('home.heroSubtitle', 'Explore our categories and deals')}</p>
        </div>
      </section>
        ...
        {categories.map((category) => (
          ...
          >
            <div className="aspect-[4/3] overflow-hidden">
              <div className="absolute inset-0 bg-blue-600 dark:bg-gray-800 opacity-90 z-10" />
              <img
                src={category.image}
                alt={category.name}
      ...
      <section className="py-12">
        <h2 className="text-3xl font-bold mb-8 text-black dark:text-white">{t('home.bestSellers')}</h2>
      ...
      <section className="py-12">
        <h2 className="text-3xl font-bold mb-8 text-black dark:text-white">{t('home.recentlyPurchased')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
